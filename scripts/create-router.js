 ...
 * @LastEditTime: 2025-04-02 16:04:00
...
// 创建文件夹和文件的函数 (异步)
async function createFiles(dir, componentName, fullPath) {
  try {
    ...
    const routerConfig:IFRouterConfig[] = [
      {
        name: '${componentName}', // 当前文件名
        path: '${componentName}', // 使用当前文件名作为路径，routerV6的字路径不需要开头'/'
        fullPath: '/${fullPath}', // 使用完整路径作为生成枚举路径
    ...
    await fs.writeFile(
      path.join(dir, "index.module.scss"),
      scssTemplate,
      "utf8"
    );
...
async function main() {
  try {
    ...
    const componentName = path.basename(targetDir); // 获取最后一级目录名作为组件名
    ...
    try {
      ...
      if (error.code === "ENOENT") {
        // 目录不存在，直接创建目标目录和文件
        await createFiles(targetDir, componentName, directory);
